<!--
Sync Impact Report:
- Version change: 3.0.0 → 3.1.0 (minor bump for Microservices Refactor & Dapr Integration principles)
- Modified principles:
  - I. Full-Stack Web Application with Separation of Concerns -> Microservices Architecture with Clear Service Boundaries
  - VII. MCP Integration for Documentation and Authentication -> MCP and Dapr Integration for Communication
- Added principles:
  - XIV. Shared Models Contract
  - XV. Dapr for Inter-Service Communication
...
- Templates requiring updates:
  ...
  - .gemini/commands/sp.constitution.md: ⚠ pending (CLAUDE.md -> GEMINI.md)
  - .gemini/commands/sp.plan.md: ⚠ pending (update-agent-context.sh claude -> gemini)
  - README.md: ⚠ pending (Claude -> Gemini references)
...
# Evo-TODO Constitution (Microservices Refactor)

## Core Principles

### I. Microservices Architecture with Clear Service Boundaries
- **Todo Service**: Exclusive handling of Task CRUD and User Auth.
- **Agent Service**: Dedicated to LLM interactions (Gemini 3 Flash), context windowing, and summarization.
...
This architecture ensures fault isolation, scalability, and maintainability through decoupled services.
...
### VII. MCP and Dapr Integration for Communication
...
- **Dapr**: Mandated for all inter-service communication (Service Invocation, Pub/Sub).
These integrations provide authoritative guidance and a robust communication layer.
...
### XIV. Shared Models Contract
Shared data models, especially SQLModel definitions, MUST be centrally defined within the `packages/shared-models/` directory to ensure type safety and consistency across all services.

### XV. Dapr for Inter-Service Communication
All communication between distinct microservices (e.g., Todo Service and Agent Service) MUST occur via Dapr Service Invocation for direct calls and Dapr Publish/Subscribe for asynchronous events. Direct HTTP calls or module imports between services are prohibited.

## Technology Stack (Microservices Refactor)

- **Frontend**: Next.js 16+ (App Router)
- **Backend**: FastAPI (async Python) microservices (Todo and Agent Services)
- **ORM**: SQLModel (type-safe, Pydantic-integrated)
- **Database**: Neon Serverless PostgreSQL
- **Authentication**: Better Auth with JWT
- **Inter-Service Communication**: Dapr (service invocation, pub/sub)
...
8. **Code Review**: All PRs reviewed for clean code adherence, task mapping, authentication/authorization, MCP compliance, performance considerations, and microservices statelessness.

## Git Branching Strategy (Microservices Refactor)

- **Main Release Branch**: `phase-3` (final merged branch for Microservices Refactor)
- **Feature Branches**: `feature/XXX-feature-name` (e.g., `feature/001-dapr-pubsub`, `feature/002-agent-service`)
...
- All code MUST reference a Task ID (in commit messages and PR descriptions, and file content).
- All inter-service communication MUST use Dapr Service Invocation or Pub/Sub.
- Shared data models (e.g., SQLModel definitions) MUST reside in `packages/shared-models`.
- SQLModel + Neon Serverless PostgreSQL for all persistent data (no in-memory-only storage).
...
All PRs and reviews MUST verify compliance with these principles. Use `GEMINI.md` for agent-specific guidance on development workflows. Violations are non-negotiable and must be remediated before merge.

**Version**: 3.1.0 (Microservices Refactor) | **Ratified**: 2025-12-27 | **Last Amended**: 2026-02-04
